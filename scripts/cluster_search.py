...
ПРАВИЛА ОЦЕНКИ:
1. Различай ФУНКЦИЮ от ПАРАМЕТРА. Функция — это что система ДЕЛАЕТ (например «расчёт загрузки по оборудованию»). Параметр — это что система УЧИТЫВАЕТ при расчёте (например «учёт фонда рабочего времени»). Маппинг делается на функцию → «прямое». Параметр может быть «дополнительный», но не «прямое».
2. Корневые пункты банка (например [4.6] без подпунктов, [5] Исполнение производства) слишком общие. Если есть более конкретный подпункт в списке кандидатов — корневой пункт = «не релевантно».
