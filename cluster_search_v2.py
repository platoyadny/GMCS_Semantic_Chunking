...
INDEX_BANK          = "processscout_bank_v2"
...
def dedup_by_bank_id(hits: list[dict]) -> list[dict]:
    """
    Дедупликация: в bank_v2 один bank_id имеет несколько вариантов (depth 0,1,2...).
    Оставляем вариант с максимальным cosine_score для каждого bank_id.
    """
    from collections import defaultdict
    by_bank = defaultdict(list)
    for h in hits:
        # В bank_v2 id документа = variant_key (например "4.7__d1")
        # bank_id хранится в metadata
        bank_id = h.get("metadata", {}).get("bank_id", h["id"].split("__")[0])
        by_bank[bank_id].append(h)

    deduped = []
    for bank_id, variants in by_bank.items():
        best = max(variants, key=lambda x: x["cosine_score"])
        best_copy = dict(best)
        best_copy["id"] = bank_id
        best_copy["_peak_depth"] = best.get("metadata", {}).get("depth", 0)
        best_copy["_peak_variant_key"] = best.get("id", "")
        deduped.append(best_copy)

    deduped.sort(key=lambda x: x["cosine_score"], reverse=True)
    return deduped
...
def compute_cluster_stats(
    ...
    for cid in cluster_ids:
        ...
        combined_score = round(0.3 * centroid_cosine + 0.5 * top3_mean + 0.2 * density, 6)
...
def call_gpt_step1(query_text: str, cluster_docs: list[dict], cluster_rank: int, openai_key: str) -> dict:
    """
    Ступень 1: LLM выбирает лучший пункт ВНУТРИ одного кластера.
    Вызывается для каждого из top-N кластеров отдельно.
    """
    from openai import OpenAI
    oa_client = OpenAI(api_key=openai_key)
...
ЗАДАЧА: Из списка пунктов банка функциональности выбери ОДИН, наиболее соответствующий описанному шагу бизнес-процесса.
...
ПРАВИЛА ВЫБОРА:
- Выбирай пункт, который описывает ту же функцию что и шаг БП
...
- Не все пункты в списке релевантны — выбери именно тот, который функционально ближе всего к шагу БП

Верни ответ строго в формате JSON (без markdown):
{{"best_id": "<ID выбранного пункта>", "explanation": "<краткое объяснение на русском, 1-2 предложения>"}}"""
...
def call_gpt_step2(query_text: str, candidates: list[dict], openai_key: str) -> dict:
    ...
    Возвращает ≥1 релевантных с оценкой типа соответствия.
    """
    from openai import OpenAI
    oa_client = OpenAI(api_key=openai_key)
    ...
    docs_text = ""
    for i, doc in enumerate(candidates, 1):
        source_label = doc.get("_source_label", f"кластер #{doc.get('_cluster_rank', '?')}")
        docs_text += (
            f"\n--- Кандидат {i} (ID: {doc['id']}, источник: {source_label}) ---\n"
            f"{doc['chunk_text_plain']}\n"
        )

    prompt = f"""Ты — эксперт по автоматизации производственных предприятий (PLM/ERP/APS/MES системы).

ЗАДАЧА: Оцени каждый из предложенных пунктов банка функциональности на соответствие шагу бизнес-процесса. Выбери ВСЕ релевантные (минимум один).

ШАГ БИЗНЕС-ПРОЦЕССА:
{query_text}

КАНДИДАТЫ (лучшие из разных тематических групп):
{docs_text}

Для каждого кандидата определи тип соответствия:
- "прямое" — описывает ту же функцию что и шаг БП (основной маппинг)
- "дополнительный" — описывает другой аспект той же операции, который может быть нужен заказчику (расширение функциональности)
- "не релевантно" — не связан с шагом БП

ПРАВИЛА:
- Должен быть выбран минимум один кандидат как "прямое" или "дополнительный"
- Пункты имеют иерархию: если кандидат является подпунктом другого кандидата, предпочитай родителя
- Оценивай по смысловому соответствию функций, а не по совпадению слов

Верни ответ строго в формате JSON (без markdown):
{{"results": [
  {{"id": "<ID>", "relevance": "прямое|дополнительный|не релевантно", "explanation": "<1-2 предложения>"}},
  ...
]}}"""
    ...
    try:
        ...
        return {"results": [{"id": candidates[0]["id"], "relevance": "прямое", "explanation": f"Ошибка парсинга: {raw}"}]}
...
def process_chunk(chunk: dict, query_embedding: list[float], client, openai_key: str, args) -> dict:
    ...
    hits_raw = dedup_by_bank_id(hits_raw)
    ...
    # Score clusters first, then number by score (cluster_1 = best)
    pool_scores_pre = [h["cosine_score"] for h in hits]
    density_threshold_pre = float(np.percentile(pool_scores_pre, 100 - args.density_threshold))

    pre_stats = compute_cluster_stats(
        hits, labels, embeddings, query_embedding,
        density_threshold=density_threshold_pre,
    )
    pre_stats.sort(key=lambda s: -s["combined_score"])

    clusters_formed: dict[str, list[str]] = {}
    cluster_id_to_rank = {}
    for rank, st in enumerate(pre_stats, 1):
        cid = st["cluster_id"]
        cluster_id_to_rank[cid] = rank
    ...
    # ================================================================
    # СТАНДАРТНЫЙ ПУТЬ: кластеры есть — двухступенчатый LLM отбор
    # ================================================================
    ...
    # ---- Ступень 1: LLM выбирает лучший пункт из каждого из top-N кластеров ----
    step1_candidates = []
    step1_details = []

    for cluster_rank_idx, cluster_stat in enumerate(ranked):
        cluster_rank = cluster_rank_idx + 1
    ...
    for outlier in qualified_outliers:
        candidate = dict(outlier)
        candidate["_cluster_rank"] = 0
        candidate["_source_label"] = "выброс (outlier)"
        candidate["_step1_explanation"] = "Прошёл фильтр выбросов"
        step1_candidates.append(candidate)

    # ---- Ступень 2: LLM оценивает всех кандидатов одновременно ----
    gpt_s2 = call_gpt_step2(query_text, step1_candidates, openai_key)
    step2_results = gpt_s2.get("results", [])

    # Находим primary (первый "прямое") и все дополнительные
    primary_doc = None
    secondary_docs = []
    not_relevant = []

    for s2r in step2_results:
        s2_id = s2r.get("id", "")
        s2_rel = s2r.get("relevance", "не релевантно")
        s2_expl = s2r.get("explanation", "")
        doc = next((c for c in step1_candidates if c["id"] == s2_id), None)
        if not doc:
            continue
        entry = {
            **clean_doc(doc),
            "relevance": s2_rel,
            "step2_explanation": s2_expl,
            "step1_explanation": doc.get("_step1_explanation", ""),
            "cluster_rank": doc.get("_cluster_rank", 0),
        }
        if s2_rel == "прямое" and primary_doc is None:
            primary_doc = entry
        elif s2_rel == "дополнительный":
            secondary_docs.append(entry)
        else:
            not_relevant.append(entry)

    # Fallback: если LLM не пометил ничего как "прямое", берём первого кандидата
    if primary_doc is None and step1_candidates:
        fallback = step1_candidates[0]
        primary_doc = {
            **clean_doc(fallback),
            "relevance": "прямое (fallback)",
            "step2_explanation": "LLM не выбрал прямого соответствия, взят лучший кандидат",
            "step1_explanation": fallback.get("_step1_explanation", ""),
            "cluster_rank": fallback.get("_cluster_rank", 0),
        }

    is_outlier_win = primary_doc and primary_doc.get("cluster_rank", 0) == 0
...
def parse_args():
    ...
    # ----- Отбор кластеров -----
    parser.add_argument(
        "--top_clusters", type=int, default=5,
        help="Сколько лучших кластеров обрабатывать (ступень 1 — LLM выбирает лучший из каждого). (по умолч.: 5)"
    )
    parser.add_argument(
...
def main():
    ...
    for i, chunk in enumerate(asis_chunks, 1):
        ...
        try:
            ...
            if status == "ok":
                ...
                n_secondary = len(entry["result"].get("secondary_documents", []))
                print(f"  ✓ {status} | {best['id']} ({best.get('relevance','?')}) "
                      f"| +{n_secondary} доп. | источник: {entry['result']['best_document_source']}")
                ok_count += 1
            else:
                print(f"  — {status}")
                skip_count += 1
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        json.dump(output, f, indent=2, ensure_ascii=False, default=lambda o: int(o) if hasattr(o, 'item') else float(o) if hasattr(o, 'is_integer') else str(o))
